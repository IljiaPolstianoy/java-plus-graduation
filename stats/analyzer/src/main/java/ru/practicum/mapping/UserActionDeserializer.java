package ru.practicum.mapping;

import org.apache.avro.io.BinaryDecoder;
import org.apache.avro.io.DatumReader;
import org.apache.avro.io.DecoderFactory;
import org.apache.avro.specific.SpecificDatumReader;
import org.apache.kafka.common.serialization.Deserializer;
import ru.practicum.error.DeserializationException;
import ru.practicum.ewm.stats.avro.UserActionAvro;

import java.util.Map;

public class UserActionDeserializer implements Deserializer<UserActionAvro> {

    private final DecoderFactory decoderFactory = DecoderFactory.get();
    private final DatumReader<UserActionAvro> reader;

    public UserActionDeserializer() {
        this.reader = new SpecificDatumReader<>(UserActionAvro.class);
    }

    public UserActionDeserializer(final DatumReader<UserActionAvro> reader) {
        this.reader = reader;
    }

    @Override
    public void configure(final Map<String, ?> configs, final boolean isKey) {
    }

    @Override
    public UserActionAvro deserialize(final String topic, final byte[] data) {
        if (data == null || data.length == 0) {
            return null;
        }

        try {
            final BinaryDecoder decoder = decoderFactory.binaryDecoder(data, null);
            return reader.read(null, decoder);
        } catch (Exception e) {
            throw new DeserializationException(
                    String.format("Ошибка десериализации UserActionAvro из топика [%s]", topic),
                    e
            );
        }
    }

    @Override
    public void close() {
    }
}